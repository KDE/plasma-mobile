...
{
    Q_UNUSED(args);
    d = new BookmarksEnginePrivate;
    d->queryClient = 0;
    setMaxSourceCount(RESULT_LIMIT); // Guard against loading too many connections
    init();
}

QString BookmarksEngine::icon(const QStringList &types)
{
    if (!d->icons.size()) {
...
bool BookmarksEngine::query(Nepomuk::Query::Query &searchQuery)
{
  searchQuery.setLimit(RESULT_LIMIT);
  return d->queryClient->query(searchQuery);
}

QStringList BookmarksEngine::sources() const
{
    return QStringList();
}
...
void BookmarksEngine::newEntries(const QList< Nepomuk::Query::Result >& entries)
{
...
void BookmarksEngine::addResource(Nepomuk::Resource resource)
{
