 ...
 */
void TouchRegistry::dispatchPointsToItem(const QTouchEvent *event, const QList<int> &touchIds, QQuickItem *item)
{
    ...
    QTouchEvent *eventForItem = new QTouchEvent(event->type(), event->device(), event->modifiers(), touchPointStates, touchPoints);
    ...
    UG_DEBUG << "Sending unowned" << qPrintable(touchEventToString(eventForItem)) << "to" << item;
...
void TouchRegistry::addCandidateOwnerForTouch(int id, QQuickItem *candidate)
{
    ...
    candidateInfo.inactivityTimer = new CandidateInactivityTimer(id, candidate, m_timerFactory->createTimer(), this);
    connect(candidateInfo.inactivityTimer.data(), &CandidateInactivityTimer::candidateDefaulted, this, &TouchRegistry::rejectCandidateOwnerForTouch);

    touchInfo->candidates.append(candidateInfo);

    connect(candidate, &QObject::destroyed, this, [=]() {
...
void TouchRegistry::removeCandidateOwnerForTouchByIndex(Pool<TouchRegistry::TouchInfo>::Iterator &touchInfo, int candidateIndex)
{
    // TODO: check if the candidate is in fact the owner of the touch
    ...
    for (int i = 0; i < touchInfo->candidates.count() && rejectedCandidateIndex == -1; ++i) {
        ...
        if (candidateInfo.item == candidate) {
            ...
            if (i == 0 && candidateInfo.state != CandidateInfo::Undecided) {
                qCritical() << "TouchRegistry: Can't reject item (" << (void *)candidate << ") as it already owns touch" << id;
                return;
            } else {
...
void TouchRegistry::TouchInfo::notifyCandidatesOfOwnershipResolution()
{
    ...
    UG_DEBUG << "sending TouchOwnershipEvent(id =" << id << " gained) to candidate" << candidates[0].item;
    ...
    for (int i = 1; i < items.count(); ++i) {
        UG_DEBUG << "sending TouchOwnershipEvent(id =" << id << " lost) to candidate" << items[i];
